...
if  (is_numeric($_POST[numberofplayers]))
{
  $sql="INSERT INTO Games (gamename,maxplayers)
  VALUES
  ('$_POST[name]','$_POST[numberofplayers]')";
  ...
  if (!$error)
    {
    $members_result = mysqli_query($con,"SELECT * FROM Members");
  
    while($row = mysqli_fetch_array($members_result))
      {
  	  for ( $c_i=$_POST[numberofplayers]; $c_i>1 ; $c_i--)
  	    {
  	    $stmt = mysqli_prepare($con,"INSERT INTO Rankings (membername,gamename,numberofplayers,rank) VALUES (?,?,?,0)");
        mysqli_stmt_bind_param($stmt,'ssi',$row['membername'],$_POST[name],$c_i);
...
{
    echo "<div style=\"color:red\">";
    echo ('Error: Max players should be a number');
    echo "</div>";
